...
document.addEventListener('DOMContentLoaded', function () {
    ...
    try {
      ...
      if (raw) {
        ...
        return {
          ...
          batchSize:     parseInt(parsed.batchSize, 10) || DEFAULT_SETTINGS.batchSize
        };
      }
    } catch (e) { /* ignore */ }
  ...
  const settingsBatchSize   = document.getElementById('settingsBatchSize');
  ...
  function populateSettingsFields() {
    ...
    settingsBatchSize.value   = currentSettings.batchSize;
  }
  ...
  settingsSave.addEventListener('click', function () {
    const rawBatch = parseInt(settingsBatchSize.value, 10);
    currentSettings = {
      ...
      batchSize:     (rawBatch >= 1 && rawBatch <= 20) ? rawBatch : DEFAULT_SETTINGS.batchSize
    };
  ...
  function renderResults(data, docId, elapsedSec) {
    ...
    if (extracted.client_name)   resultsGrid.appendChild(makeResultItem('Client Name',    extracted.client_name));
    if (extracted.document_type) resultsGrid.appendChild(makeResultItem('Document Type',  extracted.document_type));
    if (extracted.document_date) resultsGrid.appendChild(makeResultItem('Date',           extracted.document_date));
    if (extracted.amount !== undefined && extracted.amount !== null)
                                 resultsGrid.appendChild(makeResultItem('Amount',          extracted.amount));
    if (extracted.summary)       resultsGrid.appendChild(makeResultItem('Summary',         extracted.summary));
    ...
    if (errors.length > 0) {
      ...
      errSection.innerHTML = '<div class="errors-title">Validation Errors</div>' +
        errors.map(function (err) { return '<span class="error-tag">' + err + '</span>'; }).join('');
  ...
  function setTimelineStep(name) {
    timelineSteps.forEach(function (step) { step.classList.remove('active', 'done'); });
  ...
  function markAllDone() {
    setTimeout(function () {
      timelineSteps.forEach(function (step) { step.classList.remove('active'); step.classList.add('done'); });
    }, 600);
  }
  ...
  function parseCSV(text) {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (row.document_text) rows.push(row);
    }
    return rows;
  }
  ...
  async function processOneDocument(row, index) {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        return { document_id: row.document_id || ('DOC-' + Date.now() + '-' + index), client_name: row.client_name || '', document_type: '', date: '', amount: '', risk_level: '', status: 'ERROR', errors: ['HTTP ' + resp.status] };
      }
    } catch (rowErr) {
      return { document_id: row.document_id || ('DOC-' + Date.now() + '-' + index), client_name: row.client_name || '', document_type: '', date: '', amount: '', risk_level: '', status: 'ERROR', errors: [rowErr.message] };
    }
  }

  // ── Stop button hover handlers (registered once, not per-run) ────────
  ...
  batchSubmitBtn.addEventListener('click', async function () {
    ...
    try {
      ...
      const BATCH_SIZE = currentSettings.batchSize;  // ← from settings
      const batchStartTime = Date.now();

      batchProgressText.textContent = 'Processing 0 of ' + total + ' documents (batch size: ' + BATCH_SIZE + ')...';

      for (let i = 0; i < total; i += BATCH_SIZE) {
        const chunk = rows.slice(i, i + BATCH_SIZE);
        const chunkResults = await Promise.all(
          chunk.map(function (row, chunkIdx) { return processOneDocument(row, i + chunkIdx); })
        );
        chunkResults.forEach(function (r) { batchResults.push(r); });
      ...
      if (!batchCancelled) batchProgressText.textContent = 'Processing complete — ' + batchResults.length + ' documents';
      renderBatchResults();
      batchSubmitBtn.classList.remove('loading');
      batchSubmitBtn.disabled = false;
    ...
    batchResults.forEach(function (r) {
      ...
      tr.innerHTML =
        ...
        '<td class="cell-errors">' + (r.errors && r.errors.length > 0
          ? r.errors.map(function (e) { return '<span class="cell-error-tag">' + escapeHtml(e) + '</span>'; }).join('')
          : '—') + '</td>';
  ...
  exportBtn.addEventListener('click', function () {
    ...
    batchResults.forEach(function (r) {
      csv += [
        csvEscape(r.document_id), csvEscape(r.client_name), csvEscape(r.document_type),
        csvEscape(r.date), csvEscape(String(r.amount)), csvEscape(r.risk_level),
        csvEscape(r.status), csvEscape((r.errors || []).join('; '))
      ].join(',') + '\n';
    });
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    const url  = URL.createObjectURL(blob);
    const a    = document.createElement('a');
    a.href = url; a.download = 'batch-results-' + Date.now() + '.csv';
